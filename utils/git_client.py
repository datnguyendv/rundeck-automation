...
class GitClient:
    ...
    def clone(self, local_path: Path, branch: str = "main", depth: int = 1) -> Repo:
        try:
            ...
            logger.info(f"Latest commit: {repo.head.commit.hexsha[:7]} - {repo.head.commit.message.strip()}")
    ...
    def push(self, repo_path: Path, branch: Optional[str] = None, force: bool = False) -> bool:
        try:
            repo = self.get_repo(repo_path)
    ...
    def commit_and_push(self, repo_path: Path, file_path: str, commit_message: str, 
