...
Examples:
  ...
  python clean-job.py --job-href "@data.href@" --path "/tmp/@job.id@/@job.execid@"
    ...
    parser.add_argument("--job-href", help="Rundeck job href/permalink to delete")
    parser.add_argument("--job-id", help="Rundeck job ID to delete")
    parser.add_argument("--path", help="File or directory path to delete")
...
def delete_rundeck_job(job_href: str, job_id: Optional[str]) -> bool:
    logger.info("=" * 60)
